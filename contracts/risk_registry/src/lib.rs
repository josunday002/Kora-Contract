...
impl RiskRegistryContract {
    ...
    pub fn increment_invoice_count(
        ...
        env.storage()
            ...
            .set(&DataKey::SmeProfile(sme.clone()), &profile);
        Self::bump_persistent(&env, &DataKey::SmeProfile(sme.clone()));
        // AUDIT FIX: Added event emission on state change for audit trail completeness
        events::sme_invoice_counted(&env, &sme, profile.total_invoices);
        Ok(())
    }
...
mod tests {
    ...
    fn test_increment_invoice_count_emits_event() {
        // AUDIT FIX: Test for event emission on invoice count increment
        let (env, admin, invoice_nft, client) = setup();
        let verifier = Address::generate(&env);
        let sme = Address::generate(&env);
        client.add_verifier(&admin, &verifier);
        ...
        // Increment should succeed and emit an event
        assert!(client.try_increment_invoice_count(&invoice_nft, &sme).is_ok());
        assert_eq!(client.get_sme_profile(&sme).total_invoices, 1);
    }
